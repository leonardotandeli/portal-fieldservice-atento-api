...
func (repositorio Usuarios) Buscar(nomeOuLogin string) ([]modelos.Usuario, error) {
	...
	linhas, erro := repositorio.db.Query(
		"SELECT U.IDUSUARIO, U.NOME, U.RE, U.LOGIN_NT, U.CARGO, U.EMAIL, U.V_USUARIOS, U.V_BDC_POSTS, U.V_BDC_ADM, U.V_IMDB, U.V_GSA, U.V_MAPA_OPERACIONAL, U.V_MAPA_OPERACIONAL_ADM, U.ID_SITE, U.STATUS, S.IDSITE, S.NOME FROM USUARIOS U INNER JOIN SITES S ON S.IDSITE = U.ID_SITE WHERE U.NOME LIKE ? or U.LOGIN_NT LIKE ? ORDER BY U.IDUSUARIO DESC", nomeOuLogin, nomeOuLogin)
...
func (repositorio Usuarios) Atualizar(ID uint64, usuario modelos.Usuario) error {
	...
	if _, erro = statement.Exec(usuario.NOME, usuario.RE, usuario.LOGIN_NT, usuario.CARGO, usuario.EMAIL, usuario.V_USUARIOS, usuario.V_BDC_POSTS, usuario.V_BDC_ADM, usuario.V_IMDB, usuario.V_GSA, usuario.V_MAPA_OPERACIONAL, usuario.V_MAPA_OPERACIONAL_ADM, usuario.ID_SITE, usuario.STATUS, ID); erro != nil {
		return erro
	}
...
func (repositorio Usuarios) BuscarPorEmail(email string) (modelos.Usuario, error) {
	...
	if linha.Next() {
		if erro = linha.Scan(&usuario.IDUSUARIO, &usuario.LOGIN_NT, &usuario.NOME, &usuario.CARGO, &usuario.EMAIL, &usuario.V_USUARIOS, &usuario.V_BDC_POSTS, &usuario.V_BDC_ADM, &usuario.V_IMDB, &usuario.V_GSA, &usuario.V_MAPA_OPERACIONAL, &usuario.V_MAPA_OPERACIONAL_ADM, &usuario.STATUS, &usuario.Site.IDSITE, &usuario.Site.NOME, &usuario.SENHA); erro != nil {
			return modelos.Usuario{}, erro
		}
	}
